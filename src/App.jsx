          ...
          <div style={{
                          ...
                          {loading && games.length === 0 ? (
                                      ...
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                          {filteredGames.map(game => (
                                                          <div key={game.id}>
                                                                                <GameCard game={game} expanded={expandedGame === game.id} onToggle={() => setExpandedGame(expandedGame === game.id ? null : game.id)} watchlist={watchlist} onToggleWatchlist={toggleWatchlist} injuries={injuries} gameLineHistory={gameLineHistory} setPendingBet={handleSetPendingBet} />
                                                              {expandedGame === game.id && (
                                                                                  <GameDetails game={game} injuries={injuries} gameLineHistory={gameLineHistory} manualOpeners={manualOpeners} setManualOpeners={setManualOpeners} historicOdds={historicOdds} enabledBooks={enabledBooks} setPendingBet={handleSetPendingBet} />
                                                                                )}
                                                          </div>div>
                                                        ))}
                                          {filteredGames.length === 0 && !loading && (
                                                          <div style={{ textAlign: 'center', padding: '40px', color: '#64748b' }}>No games found. Try adjusting filters.</div>div>
                                                        )}
                                      </div>div>
                                    )}
                      </div>div>
              ...
              {activeTab === 'RESEARCH' && (
                      tier === 'pro' ? (
                                    ...
                                    <div style={{ padding: '40px 24px', textAlign: 'center' }}>
                                                    ...
                                                    <div style={{ fontSize: '18px', fontWeight: 700, color: '#e2e8f0', marginBottom: '8px' }}>Game Research — Pro Feature</div>div>
                                                    <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '24px' }}>Deep-dive into team trends, H2H records, and player props for every game.</div>div>
                                                    <ProBanner />
                                    </div>div>
                                  )
                    )}
              ...
              {activeTab === 'SETTINGS' && (
                      <div style={{ padding: '20px 24px', maxWidth: '600px' }}>
                                    <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '20px', color: '#f8fafc' }}>Settings</h2>h2>
                                    ...
                                    <div style={{ padding: '16px', background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(71,85,105,0.2)', borderRadius: '12px', marginBottom: '12px' }}>
                                        ...
                                        {tier === 'pro' ? (
                                        <div>
                                                        <div style={{ fontSize: '14px', color: '#c4b5fd', fontWeight: 700, marginBottom: '8px' }}>You are on Edge Finder Pro</div>div>
                                                        <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '12px' }}>All features unlocked. Thank you for supporting Edge Finder!</div>div>
                                                        <a href="https://billing.stripe.com/p/login/live" target="_blank" rel="noopener" style={{ padding: '8px 16px', background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.4)', borderRadius: '6px', color: '#818cf8', fontSize: '11px', textDecoration: 'none' }}>Manage Subscription</a>a>
                                        </div>div>
                                      ) : <ProBanner />}
                                    </div>div>
                                ...
                                <div style={{ padding: '16px', background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(71,85,105,0.2)', borderRadius: '12px', marginBottom: '12px' }}>
                                            ...
                                            <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '8px' }}>Live games refresh every 60s. Non-live every 120s.</div>div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                          <span style={{ fontSize: '12px', color: isConnected ? '#10b981' : '#ef4444' }}>{isConnected ? 'Connected' : 'Disconnected'}</span>span>
                                ...
                                <div style={{ padding: '16px', background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(71,85,105,0.2)', borderRadius: '12px', marginBottom: '12px' }}>
                                            ...
                                            <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '8px' }}>{watchlist.length} game{watchlist.length !== 1 ? 's' : ''} saved</div>div>
                                    {watchlist.length > 0 && <button onClick={() => { if (confirm('Clear entire watchlist?')) setWatchlist([]); }} style={{ padding: '6px 14px', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: '6px', color: '#f87171', fontSize: '11px', cursor: 'pointer' }}>Clear Watchlist</button>button>}
                                </div>div>
                      
                                <div style={{ padding: '16px', background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(71,85,105,0.2)', borderRadius: '12px', marginBottom: '12px' }}>
                                            <div style={{ fontSize: '13px', fontWeight: 600, color: '#e2e8f0', marginBottom: '4px' }}>Clear Cached Data</div>div>
                                            <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '8px' }}>Reset line history, manual openers, and prop tracking data.</div>div>
                                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                                          ...
                                                          <button onClick={() => { if (confirm('Clear ALL local data?')) { localStorage.clear(); location.reload(); } }} style={{ padding: '6px 14px', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: '6px', color: '#f87171', fontSize: '11px', cursor: 'pointer' }}>Reset Everything</button>button>
                                            </div>div>
                                </div>div>
                      
                                <div style={{ padding: '16px', background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(71,85,105,0.2)', borderRadius: '12px', marginBottom: '12px' }}>
                                            <div style={{ fontSize: '13px', fontWeight: 600, color: '#e2e8f0', marginBottom: '8px' }}>Edge Finder Live Odds</div>div>
                                            <div style={{ fontSize: '11px', color: '#64748b', lineHeight: '1.6' }}>Real-time odds comparison across 7+ sportsbooks and 25+ sports. Track line movement, find value, and sharpen your edge.</div>div>
                                            <div style={{ marginTop: '12px', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                                                          ...
                                                          <a href="https://x.com/TROTWAM" target="_blank" rel="noopener" style={{ fontSize: '11px', color: '#6366f1', textDecoration: 'none' }}>@TROTWAM</a>a>
                                            </div>div>
                                </div>div>
