...
const BOOK_ABBREVIATIONS = {
    ...
    'Caesars': 'Csr', 'BetRivers': 'BR', 'PointsBet': 'PB',
    'Bet365': 'B365', 'WynnBET': 'Wynn', 'Unibet': 'Uni',
    'Barstool': 'BAR', 'ESPN BET': 'ESPN', 'Fanatics': 'Fan',
};
...
function getMarketDisplayName(market) {
    return MARKET_DISPLAY_NAMES[market] || market?.replace('player_', '').toUpperCase() || market;
}
...
export default function PropsView({ playerProps, loading, propHistory }) {
  ...
  // This ensures Over and Under are BOTH shown side by side per book.
  // ============================================================
        ...
        const playersMap = {};
                              ...
                              playerProps.forEach(prop => {
                                      ...
                                      if (!playersMap[playerKey].markets[marketKey]) {
                                                playersMap[playerKey].markets[marketKey] = {
                                                            line: prop.line,
                                                            over: {},   // bookTitle → price
                                                            under: {},  // bookTitle → price
                                                            books: new Set(),
                                                };
                                      }
                                      ...
                                      mkt.books.add(bookName);
                                                        ...
                                                        // Keep the best (highest) price per book per side
                                                        if (prop.outcome === 'Over') {
                                                                  if (mkt.over[bookName] == null || prop.price > mkt.over[bookName]) {
                                                                              mkt.over[bookName] = prop.price;
                                                                  }
                                                        } else if (prop.outcome === 'Under') {
                                                                  if (mkt.under[bookName] == null || prop.price > mkt.under[bookName]) {
                                                                              mkt.under[bookName] = prop.price;
                                                                  }
                                                        }

                                                        // Update line to most common value
                                                        if (prop.line != null) mkt.line = prop.line;
                              });

                              // Convert books Set to sorted Array for consistent column ordering
                              ...
                              return Object.values(playersMap).sort((a, b) =>
                                      Object.keys(b.markets).length - Object.keys(a.markets).length
                                                                        );
  }, [playerProps]);

  // All books across all players (for header)
  const allBooks = useMemo(() => {
        const books = new Set();
        playerProps.forEach(p => {
                const b = p.book || p.bookTitle || p.bookKey;
                if (b) books.add(b);
        });
  ...
  const filteredPlayers = useMemo(() => {
        ...
        if (propSearch) {
                ...
                filtered = filtered.filter(p =>
                          p.name?.toLowerCase().includes(s) || p.game?.toLowerCase().includes(s)
                                                 );
        }
        if (propFilter !== 'ALL') {
  ...
  if (loading) {
        return (
                <div style={{ padding: '20px 24px', textAlign: 'center', paddingTop: '60px' }}>
                          ...
                          <p style={{ marginTop: '16px', color: '#94a3b8' }}>Loading player props...</p>p>
                </div>div>
              );
  }
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          { label: 'Assists', value: stats.assistsProps, color: '#3b82f6' },
                              ...
                              <div key={i} style={{ padding: '14px', background: 'rgba(30,41,59,0.5)', border: '1px solid rgba(71,85,105,0.2)', borderRadius: '10px' }}>
                                            <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '4px', fontFamily: 'JetBrains Mono, monospace' }}>{stat.label}</div>div>
                                            <div style={{ fontSize: '22px', fontWeight: 700, color: stat.color }}>{stat.value}</div>div>
                              </div>div>
                            ))}
                </div>div>
                                                ...
                                                : marketKeys.filter(k => k.includes(propFilter.toLowerCase()));
                                                                                                                       ...
                                                                                                                       return (
                                                                                                                                       <div key={player.name} style={{ background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(71,85,105,0.2)', borderRadius: '12px', overflow: 'hidden' }}>
                                                                                                                                         {/* Player Header */}
                                                                                                                                                         <div
                                                                                                                                                                           ...
                                                                                                                                                                           <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                                                                                                                                                               ...
                                                                                                                                                                                               <div>
                                                                                                                                                                                                                     <div style={{ fontWeight: 600, fontSize: '14px', color: '#e2e8f0', fontFamily: 'JetBrains Mono, monospace' }}>{player.name}</div>div>
                                                                                                                                                                                                                     <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>{player.game}</div>div>
                                                                                                                                                                                                                   </div>div>
                                                                                                                                                                             </div>div>
                                                                                                                                                                           <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                                                                                                         ...
                                                                                                                                         {/* Expanded: show Over/Under table per market */}
                                                                                                                                         {isExpanded && (
                                                                                                                                                           <div style={{ padding: '0 16px 16px' }}>
                                                                                                                                                             {visibleMarkets.map(marketKey => {
                                                                                                                                                                                   ...
                                                                                                                                                                                   // Find best over and under across all books
                                                                                                                                                                                   ...
                                                                                                                                                                                   return (
                                                                                                                                                                                                             <div key={marketKey} style={{ marginTop: '12px', padding: '12px', background: 'rgba(15,23,42,0.4)', borderRadius: '8px' }}>
                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                       {/* Over / Under comparison table */}
                                                                                                                                                                                                                                       <div style={{ overflowX: 'auto' }}>
                                                                                                                                                                                                                                                                   <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px', fontFamily: 'JetBrains Mono, monospace' }}>
                                                                                                                                                                                                                                                                                                 <thead>
                                                                                                                                                                                                                                                                                                                                 <tr>
                                                                                                                                                                                                                                                                                                                                                                   <td style={{ padding: '4px 8px', color: '#64748b', fontWeight: 600, width: '60px' }}>Side</td>td>
                                                                                                                                                                                                                                                                                                                                                                   {books.map(book => (
                                                                                                                                                                                                                                                   <td key={book} style={{ padding: '4px 8px', color: '#64748b', textAlign: 'center', minWidth: '48px' }}>
                                                                                                                                                                                                                                                                                         {getBookAbbreviation(book)}
                                                                                                                                                                                                                                                                                                 ...
                                                                                                                                                                                                                                                                                                 <tbody>
                                                                                                                                                                                                                                                                                                                                 {/* Over row */}
                                                                                                                                                                                                                                                                                                                                 <tr>
                                                                                                                                                                                                                                                                                                                                                                   <td style={{ padding: '6px 8px', color: '#22c55e', fontWeight: 700 }}>▲ Over</td>td>
                                                                                                                                                                                                                                                                                                                                                                   {books.map(book => {
                                                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                                                   return (
                                                                                                                                                                                                                                                                                           <td key={book} style={{ padding: '6px 8px', textAlign: 'center', color: isBest ? '#22c55e' : price != null ? '#e2e8f0' : '#475569', fontWeight: isBest ? 700 : 400, background: isBest ? 'rgba(34,197,94,0.1)' : 'transparent', borderRadius: isBest ? '4px' : '0' }}>
                                                                                                                                                                                                                                                                                                                                   ...
                                                                                                                                                                                                                                                                                                                                   {isBest && <span style={{ marginLeft: '2px', fontSize: '9px' }}>★</span>span>}
                                                                                                                                                                                                                                                                                                                                 </td>td>
                                                                                                                                                                                                                                                                                         );
                                                                                                                                                                                                                                                                                                                                                                   })}
                                                                                                                                                                                                                                                                                                                                                                 </tr>tr>
                                                                                                                                                                                                                                                                                                                                 {/* Under row */}
                                                                                                                                                                                                                                                                                                                                 <tr>
                                                                                                                                                                                                                                                                                                                                                                   <td style={{ padding: '6px 8px', color: '#ef4444', fontWeight: 700 }}>▼ Under</td>td>
                                                                                                                                                                                                                                                                                                                                                                   {books.map(book => {
                                ...
                                <>
                                              <div style={{ position: 'relative', marginTop: '8px' }}>
                                                      ...
                                                      <div key={player.name} style={{ padding: '16px', background: 'rgba(30,41,59,0.6)', border: '1px solid rgba(71,85,105,0.3)', borderRadius: '12px' }}>
                                                                            <div style={{ fontSize: '14px', fontWeight: 600, color: '#e2e8f0' }}>{player.name}</div>div>
                                                                            <div style={{ fontSize: '11px', color: '#64748b' }}>{Object.keys(player.markets).length} markets</div>div>
                                                      </div>div>
                                                    ))}
                                                              </div>div>
                                              ...
                                              <div style={{ marginTop: '8px' }}><ProBanner /></div>div>
                                </>>
                              )}
                  </div>div>
              )}
