...
export async function getScores(
  ...
  const games: Game[] = rawData.map(
    (raw: OddsApiResponse & { completed?: boolean; scores?: { name: string; score: string }[] }) => ({
      id: raw.id,
      sport: raw.sport_key as Sport,
      homeTeam: raw.home_team,
      awayTeam: raw.away_team,
      ...
      scores: raw.scores
        ? {
            home:
              parseInt(
                raw.scores.find((s: { name: string }) => s.name === raw.home_team)?.score ?? "0"
              ) || 0,
            away:
              parseInt(
                raw.scores.find((s: { name: string }) => s.name === raw.away_team)?.score ?? "0"
              ) || 0,
          }
        : undefined,
      bookmakers: [],
...
// Fetch all active sports odds in parallel to maximize data from limited API calls
