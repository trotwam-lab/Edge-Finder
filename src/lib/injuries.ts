...
//
// NOTE: The ESPN endpoint used below is an unofficial/undocumented
// API. It may break without notice. If it does, the error is
// swallowed gracefully and an empty array is returned — callers
// should treat empty injury data as "data unavailable" rather
// than "no injuries".
// ============================================================
...
const POSITION_IMPACT: Record<string, number> = {
    // NFL
    QB: 10, RB: 6, WR: 5, TE: 4, OL: 4, DL: 4, LB: 5, CB: 5, S: 4, K: 2, P: 1,
    // NBA
    PG: 7, SG: 6, SF: 7, PF: 7, C: 8, G: 7, F: 7,
    // MLB
    SP: 9, RP: 4, CP: 5, "1B": 5, "2B": 5, "3B": 5, SS: 6,
    LF: 4, CF: 5, RF: 4, DH: 4,
    // NHL
    GK: 9, D: 6, LW: 5, RW: 5,
    // Default
...
export function calculateImpactRating(
    ...
    const positionMultiplier =
          POSITION_IMPACT[position] ?? POSITION_IMPACT.default;
    const statusMultiplier = STATUS_SEVERITY[status];
    return Math.round(positionMultiplier * statusMultiplier);
}

// ============================================================
// Impact sum helper — shared by this module and edge-calculator
// to avoid duplicating the reduce logic.
// ============================================================
export function sumImpact(injuries: Injury[]): number {
    return injuries.reduce((sum, i) => sum + i.impactRating, 0);
}
...
export async function fetchInjuries(sport?: Sport): Promise<Injury[]> {
  ...
  const injuries = await fetchESPNInjuries(sport);
    injuryCache.set(cacheKey, {
          data: injuries,
          expires: Date.now() + INJURY_CACHE_TTL,
    });
    return injuries;
}
          ...
          if (data.items) {
                    for (const teamData of data.items) {
                      ...
                      for (const player of teamData.injuries ?? []) {
                                    ...
                                    const position =
                                                    player.athlete?.position?.abbreviation ?? "default";

                                  allInjuries.push({
                                                  id: `espn-${
                                                                    player.athlete?.id ?? Math.random().toString(36).slice(2)
                                                  }`,
                                                  playerName:
                                                  ...
                                                  status,
                                                  injuryType:
                                                                    player.type?.name ?? player.details?.type ?? "Undisclosed",
                                                  description:
                                                                    player.longComment ??
                                                                    player.shortComment ??
...
// Used by the /api/injuries route to surface per-game analysis.
// Edge detection (edge-calculator.ts) uses sumImpact() directly
// to keep the two code paths consistent from a single source.
// ============================================================
export function analyzeInjuryImpact(
  ...
  const homeImpact = sumImpact(homeInjuries);
    const awayImpact = sumImpact(awayInjuries);
